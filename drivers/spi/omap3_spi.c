...
** 功能描述 :  复位SPI总线模块。
** 输　入   : 
**　　    		struct omap3_spi_slave *ds   OMAP SPI结构体
** 输　出: 
**　　　  		无
** 全局变量:
**
...
/******************************************************************************
** 函数名	  :	 spi_setup_slave
** 功能描述 :  根据输入参数初始化设置SPI总线，返回一个spi_slave操作结构体。
** 输　入   : 
**　　    		unsigned int bus		总线号
**				unsigned int cs		使能
**				unsigned int max_hz	最高频率
**				unsigned int mode		模式
** 输　出: 
**　　　  		struct spi_slave		spi_slave操作结构体
** 全局变量:
**
** 调用模块:
**				
...
** 输　入   : 
**　　    		struct spi_slave *slave	
**				unsigned int bitlen		长度
**				const void *dout			发送数据缓存
**				void *din					接收数据缓存
